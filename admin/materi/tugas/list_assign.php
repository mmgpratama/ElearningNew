				...
				<tbody>
					...
					while ($row = $res1->fetch_assoc()) { 
						...
						<tr>
							...
							<?php  
							$query2 = "SELECT DISTINCT tb_detail_tugas.id_pelajar, tb_pelajar.nama_pelajar, tb_nilai.nilai FROM tb_detail_tugas JOIN tb_pelajar ON tb_detail_tugas.id_pelajar=tb_pelajar.id_pelajar JOIN tb_nilai ON tb_nilai.id_pelajar=tb_pelajar.id_pelajar JOIN tb_tugas ON tb_nilai.id_tugas=tb_tugas.id_tugas WHERE tb_detail_tugas.id_tugas='$id_tugas'";
							$data_nilai = $mysqli->prepare($query2);
							$data_nilai->execute();
							$res2 = $data_nilai->get_result();
							$results = $res2->fetch_assoc();
							if(!empty($results))
							{
							?>
							<td class="text-center"><?php echo $results['nilai'] ?></td>
							<?php
							}
							else {
							?>
							<td class="text-center">Nilai Belum Dimasukkan</td>
							<?php
							} 
							?>
							<td class="text-center">
								...
								<a class="btn btn-sm btn-success" href="?tugas=assign_nilai&id=<?php echo $row["id_pelajar"] ?>&id_tugas=<?php echo $id_tugas; ?>" width="50"><i class="fas fa-edit"></i></a>
							</td>
						</tr>
					<?php } ?>
				</tbody>
