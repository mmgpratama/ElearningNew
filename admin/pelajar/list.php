					...
					<?php foreach ($list_pelajar as $key => $value):?>
						<tr>
							...
							<td class="text-center">
								...
								<a class="btn btn-sm btn-warning" href="?pelajar=ubah&id=<?php echo $value["id_pelajar"] ?>" width="50" onclick="return confirm('Apakah anda yakin ingin mengubah data?')"><i class="fas fa-edit"></i>	</a>
								<a class="btn btn-sm btn-danger" href="?pelajar=hapus&id=<?php echo $value["id_pelajar"] ?>" width="50" onclick="return confirm('Apakah anda yakin ingin menghapus data?')"><i class="fas fa-trash-alt"></i></a>
							</td>
						</tr>
					<?php endforeach ?>
				</tbody>
